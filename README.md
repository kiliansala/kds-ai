...
# Sync tokens from Figma → figma/
npm run tokens:sync

# Build consumable tokens artifact
npm run tokens:build

# Optional: watch figma/tokens.*.json and rebuild
...
- Inputs: `figma/tokens.primitive.json`, `figma/tokens.semantic.json`, `figma/tokens.components.json` (pulled via `npm run tokens:sync`).
